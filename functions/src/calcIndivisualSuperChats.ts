...
const rowToTsv = (row: any) => {
  return `${row.supporterChannelId}\t${row.supporterDisplayName}\t${row.thumbnail}\t${row.superChatAmount}\t${row.totalSuperChatAmount}\t${row.channelId}\t${row.channelTitle}\n`;
};
